...
class Tier7Config(GlobalConfig):
    ...
    # ─── 소스 풀 서브샘플링 ─────────────────────────────────────────────────────
    # 소스 풀이 이 값을 초과하면 데이터셋 비례 서브샘플링을 적용한다.
    # 각 window는 이미 완결된 시계열 세그먼트이므로 window 간 순서 샘플링은
    # 시계열 무결성(Rule 5)을 위반하지 않는다.
    # None으로 설정하면 전체 데이터 사용 (느림).
    MAX_SOURCE_WINDOWS = 1_000_000   # 약 50배 속도 향상
    SOURCE_SAMPLE_SEED = GlobalConfig.SEED
