...
### 4.5 임상적 시사점: 현장 실무자를 위한 해석

위의 분석 결과를 임상 현장의 언어로 번역하면 다음과 같다.

#### "T1D용 AI를 내 T2D 환자에게 써도 되는가?"

**결론: 아니다.** 그리고 그 이유가 정량적으로 확인되었다.

T1D 환자로 학습된 혈당 예측 모델(Source-Only)을 다른 집단에 적용했을 때의 오차 크기:

| 대상 집단 | 예측 오차 (MARD) | 임상적 의미 |
|---|---|---|
| T1D/T2D 혼합 (CITY) | 6.0% | 현행 CGM 기기(MARD 9~11%)보다 우수. 사용 가능. |
| T2D (ShanghaiT2DM) | 10.6% | CGM 기기 수준. 경계선. |
| 건강인~전당뇨 (Colas_2019) | **19.0%** | **SMBG 수준. AI 예측을 신뢰할 수 없음.** |

참고로, 현재 시판 CGM 기기의 정확도 기준은 MARD ≤ 10% (Dexcom G7, Abbott FreeStyle Libre 3 등)이며,
전통적 자가 혈당 측정(SMBG)의 MARD는 약 15~20%이다.

T1D 모델을 건강인~전당뇨 집단에 적용하면, **AI 예측의 정확도가 SMBG보다 나쁜 수준**으로 떨어진다.
구체적인 수치로 환산하면:

- 실제 혈당 100 mg/dL인 환자에게 AI가 **81~119 mg/dL** 범위로 예측 (MARD 19%)
- 실제 혈당 70 mg/dL(저혈당 경계)인 환자에게 AI가 **57~83 mg/dL**로 예측
- 후자의 경우, 83 mg/dL 예측 시 저혈당 경보가 발생하지 않아 **환자 안전 위협**

#### "그러면 어떻게 해야 하는가?"

| 상황 | 권고 |
|---|---|
| T2D 환자에게 CGM AI를 처방하려 할 때 | 해당 AI가 **어떤 집단의 데이터로 학습되었는지** 확인. T1D 전용이면 주의 필요. |
| T2D 학습 데이터가 포함된 AI를 사용할 때 | 데이터가 충분하면 (70명 이상 T2D) target-only 모델로도 충분한 성능. |
| T2D 학습 데이터가 극히 적은 AI를 사용할 때 | 도메인 적응 기법(CORAL, TrAdaBoost 등)이 적용되었는지 확인. 적용 시 negative transfer 방지 가능. |
| 신규 CGM 착용 환자 (데이터 축적 초기) | AI 예측을 **보조 참고용**으로만 사용. 2주 이상 데이터 축적 후 신뢰도 상승. |

#### "CGM 처방 초기에 AI 예측을 얼마나 신뢰할 수 있는가?"

학습 곡선 분석(ShanghaiT2DM)에서 다음이 확인되었다:

- 타겟 데이터 5%(약 3,600 windows ≈ 환자 5명 분량)일 때: Target-Only RMSE **28.60** (사용 불가)
- 같은 상황에서 전이학습 적용 시: TrAdaBoost RMSE **21.26** (Source-Only 수준으로 복구)

이는 **CGM 착용 초기에는 개인화 모델보다 범용 모델(또는 전이학습 모델)이 더 안전함**을 시사한다.
환자 개인 데이터가 충분히 축적된 후(약 2주, 4,000+ 측정치)에야 개인화 모델이 우위를 보인다.

#### 이 연구가 아직 답하지 못한 임상 질문

| 질문 | 현재 상태 |
|---|---|
| 저혈당 구간(<70 mg/dL)에서의 예측 정확도는? | 미측정. RMSE 평균 개선이 위험 구간에서도 유효한지 별도 검증 필요. |
| Clarke Error Grid Zone A 비율은? | 미측정. 임상 안전성 판단의 표준 지표. 후속 분석 예정. |
| 실제 환자 의사결정(인슐린 용량 조절)에 미치는 영향은? | 예측 오차의 크기가 투약 오류로 이어지는 임계점 분석 필요. |
