...
- 해석: T1D 모델을 Mixed 코호트(건강인~전당뇨)에 적용하면 약 16건 중 1건이 위험한 임상 판단을 유발
...
- Colas_2019에서 sensitivity < 0.30이면: T1D 모델이 이 코호트의 저혈당을 구조적으로 놓침
- Mixed 코호트(건강인~전당뇨)의 저혈당 패턴이 T1D와 다르기 때문
...
- **Q5 답변:** Colas_2019에서 source_only sensitivity = **0.151** (0.30 미만). T1D 모델은 이 코호트 저혈당의 85%를 놓침.
...
**Colas_2019** (도메인 갭 최대, Mixed: 건강인~전당뇨):
...
- **Q7 답변:** Colas_2019 source_only의 <70 RMSE = 41.59 (target_only의 2.6배). T1D 모델이 이 코호트의 저혈당 패턴을 심각하게 오예측.
...
- Mixed 코호트에서 다른 환자의 데이터가 개인 데이터보다 유용하다면:
  이 코호트의 혈당 패턴이 균일하여 개인화 필요성 자체가 낮을 수 있음
