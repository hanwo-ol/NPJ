"""
Tier 5: Extended Data Utilities — Q/M Static + Local Dynamic Features + Patient ID
===================================================================================
Tier 4의 3가지 한계를 해결:
  1. Q/M 정적 피처 (age, sex, HbA1c) → Global Feature 확장 (20 → 25 dim)
  2. Local Dynamic 피처 (insulin, activity, nutrition) → Specialized Model용
  3. Patient ID 보존 → 정확한 환자 단위 리샘플링

Usage:
    from tier5_data_utils import load_dataset_tier5
"""

import numpy as np
import pandas as pd
from pathlib import Path
import sys
sys.path.insert(0, str(Path(__file__).parent.parent))
from global_config import GlobalConfig
from tier3_data_utils import (
    discover_datasets, log, mape, downcast_to_float32,
    build_windows_with_features
)

# ══════════════════════════════════════════════════════
# 1. Dataset-specific Local Feature Configuration
# ══════════════════════════════════════════════════════

# Which extra columns to use from each dataset's time-augmented CSVs
LOCAL_FEATURE_MAP = {
    'AIDET1D': [],           # no usable local features
    'BIGIDEAs': ['calorie', 'protein'],
    'Bris-T1D_Open': ['insulin', 'carbs', 'hr', 'steps'],
    'CGMacros_Dexcom': ['HR', 'Calories (Activity)', 'Carbs', 'Protein', 'Fat'],
    'CGMacros_Libre':  ['HR', 'Calories (Activity)', 'Carbs', 'Protein', 'Fat'],
    'CGMND': [],             # sleep columns not useful for regression
    'GLAM': [],              # no local features in time-augmented
    'HUPA-UCM': ['heart_rate', 'steps', 'basal_rate', 'bolus_volume_delivered', 'carb_input'],
    'IOBP2': [],             # no local features in time-augmented
    'Park_2025': [],         # food columns are categorical, not useful
    'PEDAP': ['BasalRate'],  # BolusType is categorical
    'UCHTT1DM': ['Value (g)'],
}

# Cohort-level imputation defaults for Q/M
COHORT_DEFAULTS = {
    'T1DM': {'hba1c': 7.8, 'age': 35, 'sex': 0.5},
    'ND':   {'hba1c': 5.4, 'age': 40, 'sex': 0.5},
    'GDM':  {'hba1c': 5.9, 'age': 30, 'sex': 1.0},  # GDM = female
}

DATASET_COHORT = {
    'AIDET1D': 'T1DM', 'BIGIDEAs': 'ND', 'Bris-T1D_Open': 'T1DM',
    'CGMacros_Dexcom': 'T1DM', 'CGMacros_Libre': 'T1DM',
    'CGMND': 'ND', 'GLAM': 'GDM', 'HUPA-UCM': 'T1DM',
    'IOBP2': 'T1DM', 'Park_2025': 'ND', 'PEDAP': 'T1DM', 'UCHTT1DM': 'T1DM',
}


# ══════════════════════════════════════════════════════
# 2. Q/M Static Feature Loader
# ══════════════════════════════════════════════════════

def load_metadata(dset_path):
    """Load Q/M static features from {dataset}-metadata.csv.
    Returns dict: {person_id_str: {'age': float, 'sex': float, 'hba1c': float}}
    """
    ds = dset_path.name
    meta_file = dset_path / f"{ds}-metadata.csv"
    cohort = DATASET_COHORT.get(ds, 'T1DM')
    defaults = COHORT_DEFAULTS[cohort]

    if not meta_file.exists():
        return {}, defaults

    df = pd.read_csv(meta_file)

    static = {}
    for _, row in df.iterrows():
        pid = str(row.get('person_id', ''))

        # Age: parse numeric or range
        age_raw = row.get('age', None)
        if pd.notna(age_raw):
            try:
                age = float(age_raw)
            except (ValueError, TypeError):
                # Handle ranges like "35-65"
                if '-' in str(age_raw):
                    parts = str(age_raw).split('-')
                    age = (float(parts[0]) + float(parts[1])) / 2
                else:
                    age = defaults['age']
        else:
            age = defaults['age']

        # Sex: male=0, female=1
        gender_raw = str(row.get('gender', '')).lower()
        if 'female' in gender_raw:
            sex = 1.0
        elif 'male' in gender_raw:
            sex = 0.0
        else:
            sex = defaults['sex']

        # HbA1c
        hba1c_raw = row.get('hba1c_%', None)
        if pd.notna(hba1c_raw):
            try:
                hba1c = float(hba1c_raw)
            except (ValueError, TypeError):
                hba1c = defaults['hba1c']
        else:
            hba1c = defaults['hba1c']

        static[pid] = {'age': age, 'sex': sex, 'hba1c': hba1c}

    return static, defaults


# ══════════════════════════════════════════════════════
# 3. Tier 5 Dataset Loader
# ══════════════════════════════════════════════════════

def load_dataset_tier5(dset_path,
                       seed=GlobalConfig.SEED,
                       train_ratio=GlobalConfig.TRAIN_RATIO,
                       val_ratio=GlobalConfig.VAL_RATIO,
                       include_local=True):
    """
    Load dataset with:
      - Global features (20 dim) + Q/M static features (3 dim) = 23 dim
      - Local dynamic features (dataset-specific, 0~30 dim)
      - Patient IDs preserved through split

    Returns dict with X_global_{train,val,test}, X_local_{train,val,test},
    y_{train,val,test}, patient_ids_{train,val,test}, etc.
    """
    ds = dset_path.name
    aug = dset_path / f"{ds}-time-augmented"
    pfiles = sorted(aug.glob("*.csv"))
    if not pfiles:
        return None

    # Determine local feature columns
    local_cols = LOCAL_FEATURE_MAP.get(ds, []) if include_local else []

    # Load metadata for Q/M
    metadata, defaults = load_metadata(dset_path)

    # Base feature for windowing = glucose + local dynamic
    base_feat_cols = ['glucose_value_mg_dl'] + local_cols

    all_X, all_Y = [], []
    patient_ids = []
    patient_static = {}  # pid -> {age, sex, hba1c}

    for pid_int, pf in enumerate(pfiles):
        df = pd.read_csv(pf, low_memory=False)
        if 'timestamp' not in df.columns or 'glucose_value_mg_dl' not in df.columns:
            continue
        df['timestamp'] = pd.to_datetime(df['timestamp'], errors='coerce')
        df = df.dropna(subset=['timestamp', 'glucose_value_mg_dl'])

        # Ensure all feature columns exist
        for c in base_feat_cols:
            if c not in df.columns:
                df[c] = 0.0
            else:
                df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0)

        Xm, Y, fnames = build_windows_with_features(df, base_feat_cols)
        if len(Y) < 10:
            continue

        all_X.append(Xm)
        all_Y.append(Y)
        patient_ids.extend([pid_int] * len(Y))

        # Match static features from metadata
        pid_str = pf.stem  # filename without extension
        # Try to find in metadata
        static = metadata.get(pid_str, None)
        if static is None:
            # Try matching by person_id variants
            for key in metadata:
                if key in pid_str or pid_str in key:
                    static = metadata[key]
                    break
        if static is None:
            static = {'age': defaults['age'], 'sex': defaults['sex'],
                      'hba1c': defaults['hba1c']}

        patient_static[pid_int] = static

    if not all_X:
        return None

    X = np.vstack(all_X)
    Y = np.concatenate(all_Y)
    patient_ids = np.array(patient_ids)

    # Patient-level split
    unique_pids = np.unique(patient_ids)
    rng = np.random.RandomState(seed)
    rng.shuffle(unique_pids)

    n_train = int(len(unique_pids) * train_ratio)
    n_val = int(len(unique_pids) * val_ratio)

    train_pids = set(unique_pids[:n_train])
    val_pids = set(unique_pids[n_train:n_train + n_val])
    test_pids = set(unique_pids[n_train + n_val:])

    train_mask = np.isin(patient_ids, list(train_pids))
    val_mask = np.isin(patient_ids, list(val_pids))
    test_mask = np.isin(patient_ids, list(test_pids))

    # Separate Global (first 20 cols) from Local (remaining cols)
    n_global_ts = 20  # 6 glucose lookback + 14 derived
    X_global_ts = X[:, :n_global_ts]

    # Build Q/M static array (broadcast per window)
    qm = np.zeros((len(Y), 3))  # age, sex, hba1c
    for pid_int in unique_pids:
        mask = patient_ids == pid_int
        s = patient_static.get(pid_int, defaults)
        qm[mask, 0] = s['age']
        qm[mask, 1] = s['sex']
        qm[mask, 2] = s['hba1c']

    # Global = time-series 20 + Q/M 3 = 23 dim
    X_global = np.hstack([X_global_ts, qm])

    # Local = remaining columns from windowing (if any)
    if X.shape[1] > n_global_ts:
        X_local = X[:, n_global_ts:]
    else:
        X_local = np.zeros((len(Y), 0))

    global_fnames = [f'glucose_t-{5-i}' for i in range(6)] + [
        'Velocity', 'Acceleration', 'Window_Mean', 'Window_Std',
        'TIR', 'TAR', 'TBR', 'LBGI', 'HBGI', 'Window_AUC',
        'Jerk', 'SD1', 'tod_sin', 'tod_cos',
        'age', 'sex', 'hba1c'
    ]

    # Cohort one-hot (appended to global)
    cohort = DATASET_COHORT.get(ds, 'T1DM')
    cohort_vec = np.zeros((len(Y), 3))  # T1DM, ND, GDM
    if cohort == 'T1DM':
        cohort_vec[:, 0] = 1
    elif cohort == 'ND':
        cohort_vec[:, 1] = 1
    elif cohort == 'GDM':
        cohort_vec[:, 2] = 1
    X_global = np.hstack([X_global, cohort_vec])
    global_fnames += ['cohort_T1DM', 'cohort_ND', 'cohort_GDM']

    # Total global = 20 + 3(Q/M) + 3(cohort) = 26 dim

    return {
        'name': ds,
        'cohort': cohort,
        # Global features (26 dim)
        'X_global_train': downcast_to_float32(X_global[train_mask]),
        'X_global_val':   downcast_to_float32(X_global[val_mask]),
        'X_global_test':  downcast_to_float32(X_global[test_mask]),
        # Local dynamic features (0~30 dim)
        'X_local_train':  downcast_to_float32(X_local[train_mask]),
        'X_local_val':    downcast_to_float32(X_local[val_mask]),
        'X_local_test':   downcast_to_float32(X_local[test_mask]),
        # Combined (for self/pooled training)
        'X_full_train':   downcast_to_float32(np.hstack([X_global, X_local])[train_mask]),
        'X_full_val':     downcast_to_float32(np.hstack([X_global, X_local])[val_mask]),
        'X_full_test':    downcast_to_float32(np.hstack([X_global, X_local])[test_mask]),
        # Targets
        'y_train': Y[train_mask],
        'y_val':   Y[val_mask],
        'y_test':  Y[test_mask],
        # Patient IDs (preserved!)
        'patient_ids_train': patient_ids[train_mask],
        'patient_ids_val':   patient_ids[val_mask],
        'patient_ids_test':  patient_ids[test_mask],
        # Metadata
        'global_feature_names': global_fnames,
        'local_feature_names': [f'{c}_lag{i}' for c in local_cols for i in range(6)],
        'n_global_dim': X_global.shape[1],
        'n_local_dim': X_local.shape[1],
        'n_subjects': len(unique_pids),
        'n_windows': len(Y),
        'patient_static': patient_static,
    }
