...
def clean_cgmacros_data(df, subject_id, output_dir):
    '''
    Cleans and standardizes CGMacros_Dexcom CGM data by:
    ...
    '''
    df.rename(columns={"Timestamp": "timestamp", "Dexcom GL": "glucose_value_mg_dl"}, inplace=True)
    ...
    ext_dir = str(output_dir).replace("CGMacros_Dexcom-extracted-glucose-files", "CGMacros_Dexcom-extended-features")
...
def main():
    '''
    Processes raw data from the CGMacros_Dexcom dataset by pulling timestamp & glucose data and 
    ...
    if len(sys.argv) != 2:
        print("Invalid command. Usage: python CGMacros_Dexcom_extract-glucose-data.py <input_folder>")
    ...
    output_dir = glucose_ml_dir / "3_Glucose-ML-collection/CGMacros_Dexcom/CGMacros_Dexcom-extracted-glucose-files"
